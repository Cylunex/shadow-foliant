...
class NewsFlowDatabase:
    ...
    def __init__(self, db_path: str = _bootstrap.db_path("news_flow.db")):
    ...
    def _init_pg(self):
        """PG 建表(方言:BIGSERIAL/DOUBLE PRECISION/TIMESTAMP;迁移列已内联)。幂等。"""
        ddl = [
            """CREATE TABLE IF NOT EXISTS flow_snapshots (
                id BIGSERIAL PRIMARY KEY, fetch_time TEXT NOT NULL, total_platforms INTEGER NOT NULL,
                success_count INTEGER NOT NULL, total_score INTEGER NOT NULL, flow_level TEXT NOT NULL,
                social_score INTEGER, news_score INTEGER, finance_score INTEGER, tech_score INTEGER,
                analysis TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS platform_news (
                id BIGSERIAL PRIMARY KEY, snapshot_id INTEGER NOT NULL, platform TEXT NOT NULL,
                platform_name TEXT NOT NULL, category TEXT NOT NULL, weight INTEGER NOT NULL,
                title TEXT NOT NULL, content TEXT, url TEXT, source TEXT, publish_time TEXT,
                rank INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS stock_related_news (
                id BIGSERIAL PRIMARY KEY, snapshot_id INTEGER NOT NULL, platform TEXT NOT NULL,
                platform_name TEXT NOT NULL, category TEXT NOT NULL, weight INTEGER NOT NULL,
                title TEXT NOT NULL, content TEXT, url TEXT, source TEXT, publish_time TEXT,
                matched_keywords TEXT, keyword_count INTEGER, score INTEGER DEFAULT 0, rank INTEGER,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS hot_topics (
                id BIGSERIAL PRIMARY KEY, snapshot_id INTEGER NOT NULL, topic TEXT NOT NULL,
                count INTEGER NOT NULL, heat INTEGER NOT NULL, cross_platform INTEGER, sources TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS flow_statistics (
                id BIGSERIAL PRIMARY KEY, date TEXT NOT NULL UNIQUE, avg_score INTEGER, max_score INTEGER,
                min_score INTEGER, snapshot_count INTEGER, top_topics TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS sentiment_records (
                id BIGSERIAL PRIMARY KEY, snapshot_id INTEGER, sentiment_index INTEGER NOT NULL,
                sentiment_class TEXT NOT NULL, flow_stage TEXT NOT NULL, momentum DOUBLE PRECISION,
                viral_k DOUBLE PRECISION, flow_type TEXT, stage_analysis TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS flow_alerts (
                id BIGSERIAL PRIMARY KEY, alert_type TEXT NOT NULL, alert_level TEXT NOT NULL, title TEXT NOT NULL,
                content TEXT, related_topics TEXT, trigger_value TEXT, threshold_value TEXT,
                is_notified INTEGER DEFAULT 0, snapshot_id INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS ai_analysis (
                id BIGSERIAL PRIMARY KEY, snapshot_id INTEGER, affected_sectors TEXT, recommended_stocks TEXT,
                risk_level TEXT, risk_factors TEXT, advice TEXT, confidence INTEGER, summary TEXT,
                raw_response TEXT, model_used TEXT, analysis_time DOUBLE PRECISION,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS scheduler_logs (
                id BIGSERIAL PRIMARY KEY, task_name TEXT NOT NULL, task_type TEXT, status TEXT NOT NULL,
                message TEXT, duration DOUBLE PRECISION, snapshot_id INTEGER,
                executed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS alert_config (
                id BIGSERIAL PRIMARY KEY, config_key TEXT NOT NULL UNIQUE, config_value TEXT NOT NULL,
        ...
        for key, value, desc in [
            ('heat_threshold', '800', '热度飙升阈值'), ('rank_change_threshold', '10', '排名变化阈值'),
            ('sentiment_high_threshold', '90', '情绪高位阈值'), ('sentiment_low_threshold', '20', '情绪低位阈值'),
            ('viral_k_threshold', '1.5', 'K值阈值'), ('alert_enabled', 'true', '预警开关'),
            ('notification_enabled', 'true', '通知开关'),
        ]:
            cur.execute("""INSERT INTO alert_config (config_key, config_value, description)
                VALUES (?,?,?) ON CONFLICT (config_key) DO NOTHING""", (key, value, desc))
        conn.commit()
        conn.close()
        logger.info("✅ 新闻流量 PG 表初始化完成")
