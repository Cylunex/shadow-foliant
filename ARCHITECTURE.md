...
| 模块 | 入口文件 | 功能 |
|---|---|---|
| 🎯 决策信号统一层 | `analysis/decision_signal.py` | 每次分析/选股/盯盘/AI能力的操作主张抽成结构化信号(8 态动作 + 生命周期 + 去重 + 反向作废);同来源/周期只保留一个活跃状态，跨风险状态变化写服务端事件；K线方向后验按 source/动作/周期分桶。低于 30 个方向样本仅观察，达到门槛后用 Beta(15,15) 收缩，反馈权重限制在约 0.83～1.2。**全项目 AI 信号的后验底座** |
| 🧭 多周期交易计划 | `analysis/multi_timeframe.py` + `technical_state.py` + `trade_plan.py` | 自然交易周 OHLCV 聚合（周五结束）+ 周日共振；量价/OBV、排除当日的唐奇安通道、ADX/DMI、标准化趋势斜率/R²、20 日波动/最大回撤/ATR、整理突破及确认形态形成封顶软评分；再把关键价位、ATR/VaR、盈亏比 2.0 门槛和组合动作收口为 Agent 可直接消费的入场/止损/目标/仓位合同。形态测算价仅作第二目标，不参与首目标盈亏比。 |
| 🧾 Agent 数据质量 | `core/agent_contract.py` + `analysis/market_structure.py` | `research_stock` 按实际请求组计算质量分、分块状态、阶段耗时和确定性置信护栏；核心行情/K线降级会限制方向性动作。市场结构只消费已有行业/概念/热点上下文判断题材匹配和前排/跟随角色，不额外联网、不伪造题材阶段。 |
| 🌐 市场宽度 | `analysis/market_breadth.py` | 只读已缓存的中证 A500 成分并批量计算涨跌家数/中位涨跌；缓存或覆盖不足即降级，不阻塞早盘五档动作。 |
...
09:15 主力资金最高优先级，使用“主力资金净流入排名”轻问句获取真实当日
净流入/排名/买卖额字段，并在本地排除 ST/科创；所有只需 TOP5/TOP100 的问句均禁用全量分页。
...
| 时间 | 任务 | 内容 | AI |
|---|---|---|---|
| 09:00 | `morning_strategy` | ☀️ 晨间市场报告(龙虎榜/美股隔夜/新闻/北向/热点/板块/宏观/持仓；LLM 空响应/超时自动切规则版，禁止空壳推送) | ✅ |
| 09:15 | `strategy_prefetch` | 🏦 问财+妙想盘前预取 | ❌ |
| 09:30 | `strategy_prefetch_retry` | 🔁 仅补未写入缓存的问财策略 | ❌ |
| 09:45 | `unified_selection` | 🎯 保留综合选股 TOP15 完整推送，再按规则分、多源共振、红蓝复核、自然周/日线共振和追高风险独立推送最终 TOP5；最终层附确定性入场/止损/目标/盈亏比计划，两层用独立 source 跟踪纸面战绩 | ✅ |
