...
class SectorStrategyDatabase:
    ...
    def __init__(self, db_path=_bootstrap.db_path('sector_strategy.db')):
        """
        初始化数据库
    ...
    def _init_pg(self):
        ...
        ddl = [
            """CREATE TABLE IF NOT EXISTS sector_raw_data (
                id BIGSERIAL PRIMARY KEY, data_date TEXT NOT NULL, sector_code TEXT NOT NULL,
                sector_name TEXT, price DOUBLE PRECISION, change_pct DOUBLE PRECISION,
                volume DOUBLE PRECISION, turnover DOUBLE PRECISION, market_cap DOUBLE PRECISION,
                pe_ratio DOUBLE PRECISION, pb_ratio DOUBLE PRECISION, data_type TEXT,
                data_version INTEGER DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                UNIQUE(data_date, sector_code, data_type))""",
            "CREATE INDEX IF NOT EXISTS idx_sector_data_date ON sector_raw_data(data_date)",
            "CREATE INDEX IF NOT EXISTS idx_sector_code ON sector_raw_data(sector_code)",
            "CREATE INDEX IF NOT EXISTS idx_data_type ON sector_raw_data(data_type)",
            """CREATE TABLE IF NOT EXISTS sector_news_data (
                id BIGSERIAL PRIMARY KEY, news_date TEXT NOT NULL, title TEXT, content TEXT,
                source TEXT, url TEXT, related_sectors TEXT, sentiment_score DOUBLE PRECISION,
                importance_score DOUBLE PRECISION, data_version INTEGER DEFAULT 1,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS sector_analysis_reports (
                id BIGSERIAL PRIMARY KEY, analysis_date TEXT NOT NULL, data_date_range TEXT,
                analysis_content TEXT, recommended_sectors TEXT, summary TEXT,
                confidence_score DOUBLE PRECISION, risk_level TEXT, investment_horizon TEXT,
                market_outlook TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS sector_tracking (
                id BIGSERIAL PRIMARY KEY, analysis_id INTEGER, sector_code TEXT NOT NULL, sector_name TEXT,
                recommended_date TEXT, recommended_price DOUBLE PRECISION, target_price DOUBLE PRECISION,
                stop_loss_price DOUBLE PRECISION, current_price DOUBLE PRECISION, profit_loss_pct DOUBLE PRECISION,
                status TEXT, notes TEXT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""",
            """CREATE TABLE IF NOT EXISTS data_versions (
                id BIGSERIAL PRIMARY KEY, data_type TEXT NOT NULL, data_date TEXT NOT NULL,
                version INTEGER NOT NULL, status TEXT DEFAULT 'active', fetch_success BOOLEAN DEFAULT TRUE,
                error_message TEXT, record_count INTEGER DEFAULT 0,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE(data_type, data_date, version))""",
        ]
        conn = self.get_connection()
        cur = conn.cursor()
        for stmt in ddl:
