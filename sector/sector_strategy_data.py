...
class SectorStrategyDataFetcher:
    ...
    def _get_sector_performance(self):
        """获取行业板块表现（a-stock HTTP > akshare 优先级）"""
        # ★ 优先级1: a-stock HTTP直连（东财push2行业板块，零依赖）
        try:
            print("    尝试a-stock HTTP行业板块数据...")
            import datahub
            ranking = datahub.sector_ranking("industry", 100)
            if ranking and ranking.get("total", 0) > 0:
                sectors = {}
                for item in ranking.get("top", []) + ranking.get("bottom", []):
                    name = item.get('name', '')
                    if name:
                        sectors[name] = {
                            "name": name,
                            "change_pct": item.get('change_pct', 0),
                            "turnover": 0,  # push2不直接返回换手率
                            "total_market_cap": 0,
                            "top_stock": item.get('leader', ''),
                            "top_stock_change": item.get('leader_change', 0),
                            "up_count": item.get('up_count', 0),
                            "down_count": item.get('down_count', 0)
                        }
                if sectors:
                    print(f"    ✓ [a-stock HTTP] 成功获取 {len(sectors)} 个行业板块")
                    return sectors
        except Exception as ae:
            print(f"    [a-stock HTTP] 获取失败（将切akshare）: {ae}")
        
        # 优先级2: akshare 东财(本机/部分网络 push2 被墙,空则继续往下兜底)
        try:
            print("    尝试akshare(东财)行业板块数据...")
            ...
            if df is not None and not df.empty:
                ...
                for idx, row in df.iterrows():
                    ...
                    if sector_name:
                        sectors[sector_name] = {
                            "name": sector_name, "change_pct": row.get('涨跌幅', 0),
                            "turnover": row.get('换手率', 0), "total_market_cap": row.get('总市值', 0),
                            "top_stock": row.get('领涨股票', ''), "top_stock_change": row.get('领涨股票涨跌幅', 0),
                            "up_count": row.get('上涨家数', 0), "down_count": row.get('下跌家数', 0)
                        }
                if sectors:
                    return sectors
        except Exception as e:
            print(f"    [akshare东财] 失败(将切新浪/同花顺): {e}")

        # ★ 优先级3: 新浪行业(非东财,稳定可达,带领涨股)
        try:
            print("    尝试新浪行业板块数据...")
            df = self._safe_request(lambda: ak.stock_sector_spot(indicator="新浪行业"))
            if df is not None and not df.empty:
                sectors = {}
                for _, row in df.iterrows():
                    ...
                    if name:
                        sectors[name] = {
                            "name": name, "change_pct": row.get('涨跌幅', 0),
                            "turnover": 0, "total_market_cap": row.get('总成交额', 0),
                            "top_stock": row.get('股票名称', ''), "top_stock_change": row.get('个股-涨跌幅', 0),
                            "up_count": 0, "down_count": 0
                        }
                if sectors:
                    print(f"    ✓ [新浪行业] 成功获取 {len(sectors)} 个行业板块")
                    return sectors
        except Exception as e:
            print(f"    [新浪行业] 失败(将切同花顺): {e}")
                ...
                for _, row in df.iterrows():
                    ...
                    if name:
                        sectors[name] = {
                            "name": name, "change_pct": row.get('涨跌幅', 0),
                            "turnover": 0, "total_market_cap": row.get('总成交额', 0),
                            "top_stock": '', "top_stock_change": 0,
                            "up_count": 0, "down_count": 0, "net_inflow": row.get('净流入', 0)
                        }
                if sectors:
                    print(f"    ✓ [同花顺行业] 成功获取 {len(sectors)} 个行业板块")
                    return sectors
        except Exception as e:
            print(f"    [同花顺行业] 失败: {e}")
        return {}
    ...
    def _get_concept_performance(self):
        """获取概念板块表现（a-stock HTTP push2 > akshare 优先级）"""
        # ★ 优先级1: a-stock HTTP 直连（东财 push2 m:90+t:3，零鉴权）
        try:
            print("    尝试a-stock HTTP概念板块数据...")
            import datahub
            ranking = datahub.sector_ranking("concept", 200)
            if ranking and ranking.get("total", 0) > 0:
                concepts = {}
                for item in ranking.get("top", []) + ranking.get("bottom", []):
                    name = item.get('name', '')
                ...
                if concepts:
                    print(f"    ✓ [a-stock HTTP] 成功获取 {len(concepts)} 个概念板块")
                    return concepts
        except Exception as ae:
            print(f"    [a-stock HTTP] 概念板块失败（将切akshare）: {ae}")
    ...
    def _get_sector_fund_flow(self):
        """获取行业资金流向（a-stock HTTP push2 > akshare 优先级）"""
        # ★ 优先级1: a-stock HTTP 直连（东财 push2，零鉴权替代 akshare）
        try:
            print("    尝试a-stock HTTP行业资金流...")
            import datahub
            rows = datahub.sector_fund_flow("industry", 50)
            if rows:
                fund_flow = {
                    "today": [],
                ...
                for r in rows:
                    fund_flow["today"].append({
                        "sector": r.get('name', ''),
                        "main_net_inflow": r.get('main_net_inflow', 0),
                        "main_net_inflow_pct": r.get('main_net_inflow_pct', 0),
                        "super_large_net_inflow": r.get('super_large_net_inflow', 0),
                        "large_net_inflow": r.get('large_net_inflow', 0),
                        "medium_net_inflow": r.get('medium_net_inflow', 0),
                        "small_net_inflow": r.get('small_net_inflow', 0),
                        "change_pct": r.get('change_pct', 0)
                    })
                print(f"    ✓ [a-stock HTTP] 成功获取 {len(rows)} 个行业资金流")
                return fund_flow
        except Exception as ae:
            print(f"    [a-stock HTTP] 行业资金流失败（将切akshare）: {ae}")
