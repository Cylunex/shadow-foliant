...
class ValueStockSelector:
    ...
    def get_value_stocks(self, top_n: int = 10) -> Tuple[bool, Optional[pd.DataFrame], str]:
        ...
        try:
            ...
            result = pywencai_get(query, timeout=40, loop=False, retry=1, group='低估值')
            if result is None:
            ...
            for idx, row in selected.iterrows():
                # 兼容两种列名：统一数据源(code/name) / pywencai(股票代码/股票简称)
                code = row.get('股票代码', row.get('code', 'N/A'))
                name = row.get('股票简称', row.get('name', 'N/A'))
                pe = row.get('市盈率', row.get('pe', 'N/A'))
                pb = row.get('市净率', row.get('pb', 'N/A'))
                div_rate = row.get('股息率', 'N/A')
                debt_ratio = row.get('资产负债率', 'N/A')
                cap = row.get('流通市值', row.get('mcap', 'N/A'))
