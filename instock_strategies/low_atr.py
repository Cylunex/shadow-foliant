...
# 低ATR成长
# 1.必须至少上市交易 ma_long 日(默认250)
# 2.最近 threshold 日(默认10)区间日均振幅 ≤ atr_max_pct%(默认10)
# 3.区间(最高-最低)/最低 > ratio_min(默认1.1)
# 参数化(策略基因组): atr_max_pct / ratio_min,默认值=原硬编码
def check_low_increase(code_name, data, date=None, ma_short=30, ma_long=250, threshold=10,
                       atr_max_pct=10.0, ratio_min=1.1):
