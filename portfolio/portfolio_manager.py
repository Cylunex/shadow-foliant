...
class PortfolioManager:
    ...
    def analyze_single_stock(self, stock_code: str, period="1y", 
        ...
        try:
            # 共享分析逻辑已从 app.py 抽到 core/batch_analyze.py(WebUI 迁移后 app.py 已删)
            from batch_analyze import analyze_single_stock_for_batch
    ...
    def batch_analyze_sequential(self, stock_codes: List[str], period="1y",
                                 ...
                                 incremental_save: bool = True) -> Dict:
        """
        顺序批量分析（逐只分析）
        ...
        Args:
            ...
            incremental_save: 每只分析完成后立即入库（默认开），中断也不丢已分析结果

        Returns:
            批量分析结果字典
        """
        print(f"\n{'='*60}")
        print(f"开始批量分析 (顺序模式): {len(stock_codes)}只股票  incremental_save={incremental_save}")
        print(f"{'='*60}\n")
        ...
        for i, code in enumerate(stock_codes, 1):
            ...
            try:
                ...
                if result.get("success"):
                    # ⭐ 每只分析完成后立即入库，崩了也只丢正在跑的这一只
                    if incremental_save:
                        try:
                            aid = self._save_one_analysis(code, result)
                            if aid is not None:
                                result['saved_id'] = aid
                                saved_count += 1
                                print(f"   ✅ 已入库 analysis_id={aid} (累计已存 {saved_count} 条)")
                        except Exception as se:
                            print(f"   ⚠️ 增量入库失败 ({code}): {se}")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_code):
                ...
                try:
                    ...
                    if result.get("success"):
                        # ⭐ 每只完成后立即入库（PG 每次 get_conn 新连接，线程安全）
                        if incremental_save:
                            try:
                                aid = self._save_one_analysis(code, result)
                                if aid is not None:
                        ...
                        print(f"\n[{completed}/{len(stock_codes)}] {code} 分析完成 "
                              f"(saved_id={result.get('saved_id')})")
                        if progress_callback:
    ...
    def _save_one_analysis(self, code: str, result: Dict) -> Optional[int]:
        ...
        def _first_num(s):
            if not s:
                return None
            try:
                ns = re.findall(r'-?\d+\.?\d*', str(s))
                return float(ns[0]) if ns else None
            except Exception:
                return None

        target_price = _first_num(target_price_str)
        take_profit = _first_num(take_profit_str)
        stop_loss = _first_num(stop_loss_str)
        ...
        summary = (final_decision.get("advice", final_decision.get("summary", "")) or "")[:500]
    ...
    def save_analysis_results(self, analysis_results: Dict) -> List[int]:
        ...
        如果 result 已经携带 'saved_id'（说明增量入库已存过），跳过。
        否则调 _save_one_analysis 入库。
        """
        saved_ids = []
        ...
        for item in analysis_results.get("results", []):
            ...
            # 增量入库已存过的跳过（避免重复）
            if result.get('saved_id') or item.get('saved_id'):
                saved_ids.append(result.get('saved_id') or item.get('saved_id'))
                continue

            aid = self._save_one_analysis(code, result)
            if aid is not None:
                saved_ids.append(aid)
                # 回写 saved_id，再次调用也不会重复
                result['saved_id'] = aid
                item['saved_id'] = aid
