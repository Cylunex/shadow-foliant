...
> ⚠️ **本文档已过时(描述 2025 年的 Akshare+Tushare 双源架构)。** 该架构已于 2026-06 由
> 「门面 `datahub` + `_route` 健康度路由」重构取代:直连多家真源(东财/腾讯/新浪/同花顺/baostock/通达信),
> akshare/tushare 仅作各域末位兜底/可选源;`data_source_manager` 已退为 datahub 下层内部编排组件。
> **最新数据层架构以 [数据源原子化重构计划.md](数据源原子化重构计划.md) 与 [ARCHITECTURE.md](../ARCHITECTURE.md) 为准。** 下文仅留作历史参考。
...
> ⚠️ **2026-06-28 起 `data_source_manager.get_stock_hist_data` 已删除**(8 源降级链摊平进 `datahub.kline`)。
> 下方含该方法的示例**仅历史参考**;现在取 K线请用门面:
> ```python
> import datahub
> df = datahub.kline("600519", "1y", adjust="qfq")  # DatetimeIndex='Date' + 大写 OHLCV;raw/qfq 两套缓存
> ```
> `get_stock_basic_info` / `get_realtime_quotes` / `get_financial_data` 等非 K线方法仍在 `data_source_manager`。
