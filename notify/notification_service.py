...
class NotificationService:
    ...
    def _send_email_notification(self, notification: Dict) -> bool:
        ...
        try:
            ...
            msg['Subject'] = f"股票监测提醒 - {notification.get('symbol') or notification.get('name') or ''}"
...
_此消息由AI股票分析系统自动发送_"""
    ...
    # ============================================================
    # 兼容性属性 — 外部代码用 self.email_enabled / self.webhook_enabled
    # ============================================================
    @property
    ...
    def send_analysis_result(self, subject: str, content: str,
        ...
        """通用"分析完成"通知 — 同时走邮件 + webhook

        Args:
            subject: 邮件/消息标题
            content: 正文（纯文本，会自动转换 HTML）
            channels: 可选 ['email','webhook']，限制只走某些渠道
