...
# K线类型 → mootdx frequency 字符串
_KLINE_TYPE_MAP = {
    'minute1': '1m', 'minute5': '5m', 'minute15': '15m', 'minute30': '30m',
    'hour': '1h', 'day': 'day', 'week': 'week', 'month': 'month',
}
...
class SmartMonitorTDXDataFetcher:
    ...
    def __init__(self, base_url: str = None):
        ...
        Args:
            base_url: TDX HTTP API 基础地址。默认读 env TDX_HTTP_URL(可移植);
                      留空且无 env 时退回原内网地址,仅作 mootdx 不可用时的兜底。
        """
        self.logger = logging.getLogger(__name__)
        self.base_url = (base_url or os.getenv('TDX_HTTP_URL', 'http://127.0.0.1:8181')).rstrip('/')
        self.timeout = 10  # 请求超时时间（秒）
        # 默认优先用 mootdx 公网行情(纯Python、可移植);env TDX_USE_MOOTDX=false 可关
        self.use_mootdx = os.getenv('TDX_USE_MOOTDX', 'true').lower() in ('1', 'true', 'yes', 'on')

        self.logger.info(f"TDX数据源初始化成功，mootdx={'on' if self.use_mootdx else 'off'}，HTTP兜底={self.base_url}")
    ...
    def get_kline_data(self, stock_code: str, kline_type: str = 'day', limit: int = 200) -> Optional[pd.DataFrame]:
        ...
        Returns:
            K线数据DataFrame
        """
        # 优先 mootdx 公网行情(可移植,无需内网服务);失败再走 HTTP 兜底
        if self.use_mootdx:
            try:
                from tdx_mootdx import get_kline as _tdx_k
                freq = _KLINE_TYPE_MAP.get(kline_type, 'day')
                mdf = _tdx_k(stock_code, frequency=freq, count=min(limit, 800))
                if mdf is not None and len(mdf) > 0:
                    out = mdf.rename(columns={
                    ...
                    for col in ('日期', '开盘', '收盘', '最高', '最低', '成交量', '成交额'):
                        if col not in out.columns:
                            out[col] = None
                    out = out[['日期', '开盘', '收盘', '最高', '最低', '成交量', '成交额']].tail(limit).reset_index(drop=True)
                    out['日期'] = pd.to_datetime(out['日期'])
                    self.logger.info(f"✅ mootdx 获取 {stock_code} K线 {len(out)} 条")
                    return out
            except Exception as e:
                self.logger.warning(f"mootdx 取K线失败({stock_code}),回退 HTTP: {type(e).__name__}")
...
if __name__ == '__main__':
    ...
    # 使用默认地址测试
    fetcher = SmartMonitorTDXDataFetcher(base_url="http://127.0.0.1:8181")
