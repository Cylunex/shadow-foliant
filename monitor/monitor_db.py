...
from db_compat import connect as db_connect, USE_POSTGRES, coerce_json


class StockMonitorDatabase:
    """股票监测数据库管理类"""
    
    def __init__(self, db_path: str = _bootstrap.db_path("stock_monitor.db")):
    ...
    def has_recent_notification(self, stock_id: int, notification_type: str, minutes: int = 60) -> bool:
        ...
        cursor.execute('''
            SELECT COUNT(*) FROM monitor_notifications
    ...
    def add_notification(self, stock_id: int, notification_type: str, message: str):
        ...
        cursor.execute('''
            INSERT INTO monitor_notifications (stock_id, type, message)
            VALUES (?, ?, ?)
    ...
    def mark_notification_sent(self, notification_id: int):
        ...
        cursor.execute('''
            UPDATE monitor_notifications SET sent = TRUE WHERE id = ?
        ''', (notification_id,))
    ...
    def mark_all_notifications_sent(self):
        ...
        cursor.execute('UPDATE monitor_notifications SET sent = TRUE WHERE sent = FALSE')
    ...
    def clear_all_notifications(self):
        ...
        cursor.execute('DELETE FROM monitor_notifications')
    ...
    def remove_monitored_stock(self, stock_id: int):
        ...
        try:
            ...
            cursor.execute('DELETE FROM monitor_notifications WHERE stock_id = ?', (stock_id,))
