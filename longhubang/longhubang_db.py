...
def get_longhubang_db(db_path=_bootstrap.db_path('longhubang.db')):
    """龙虎榜数据库工厂 — 根据 USE_POSTGRES 开关返回 PG 或 SQLite 实例

    SQLite: 接受 db_path 参数
    PG:     忽略 db_path（连接信息来自 PG_HOST 等环境变量）
    """
    if os.getenv('USE_POSTGRES', '').lower() in ('1', 'true', 'yes', 'on'):
        try:
            from database_pg import longhubang_db as _pg_lhb
            return _pg_lhb
        except Exception as _e:
            print(f'[Longhubang] PG 切换失败，回退到 SQLite: {_e}')
    return LonghubangDatabase(db_path)


class LonghubangDatabase:
    """龙虎榜数据库管理类"""
    
    def __init__(self, db_path=_bootstrap.db_path('longhubang.db')):
        """
        初始化数据库
