...
import requests
try:
    from rate_limiter import throttle as _throttle  # 自限流,防封
except Exception:
    def _throttle(*a, **k):
        return 0.0
...
class LonghubangDataFetcher:
    ...
    def get_longhubang_data(self, date):
        ...
        # 主源(ws4.cn)挂了 → akshare 龙虎榜明细兜底(东财host,非push2,稳定可达;
        # 无营业部席位级,评分中资金含金量/机构维度降级,净买入维度仍有效)
        fb = self._fallback_akshare(date)
        if fb and fb.get('data'):
            print(f"    ✓ [akshare兜底] 成功获取 {len(fb['data'])} 条龙虎榜记录")
            return fb
        print(f"    ✗ 未获取到数据")
        return None

    def _fallback_akshare(self, date):
        """akshare 龙虎榜明细兜底(格式映射到 ws4.cn 字段)。"""
        try:
            ...
            for _, r in df.iterrows():
                rows.append({
                    'gpdm': str(r.get('代码', '')), 'gpmc': r.get('名称', ''),
                    'jlrje': r.get('龙虎榜净买额') or r.get('净买额') or 0,
                    'mrje': r.get('龙虎榜买入额') or r.get('买入额') or 0,
                    'mcje': r.get('龙虎榜卖出额') or r.get('卖出额') or 0,
                    'yzmc': '', 'yyb': '', 'sblx': r.get('上榜原因', ''),
                    'date': date,
                })
            return {'code': 0, 'data': rows, 'source': 'akshare'}
        except Exception as e:
            print(f"    [akshare兜底] 失败: {e}")
            return None
