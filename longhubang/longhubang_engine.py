...
from longhubang_db import LonghubangDatabase, get_longhubang_db
...
class LonghubangEngine:
    ...
    def __init__(self, model=None, db_path=_bootstrap.db_path('longhubang.db')):
        """
        初始化分析引擎
        ...
        self.database = get_longhubang_db(db_path)
    ...
    def _generate_final_report(self, agents_results: Dict, summary: Dict, 
        ...
        # 推送通知（邮件 + Webhook）— silent 失败不影响主流程
        try:
            self._notify_analysis_complete(report, recommended_stocks)
        except Exception as ne:
            self.logger.warning(f"[智瞰龙虎] 推送通知失败（不影响分析结果）: {ne}")

        return report

    def _notify_analysis_complete(self, report, recommended_stocks):
        """龙虎榜分析完成后推送邮件+Webhook"""
        try:
            from notification_service import notification_service
        except ImportError:
            return
        if not (notification_service.email_enabled or notification_service.webhook_enabled):
            return

        date_range = report.get('data_date_range', '')
        lines = [
            f"🐉 智瞰龙虎分析完成 — {date_range}",
            "",
            report.get('summary', ''),
            "",
            "📊 数据概况：",
            f"  总记录数：{report.get('data_summary', {}).get('total_records', 0)}",
            f"  涉及股票：{report.get('data_summary', {}).get('total_stocks', 0)} 只",
            f"  涉及游资：{report.get('data_summary', {}).get('total_youzi', 0)} 席",
            "",
            f"🎯 AI 推荐潜力股 (共 {len(recommended_stocks)} 只)：",
        ]
        for i, s in enumerate(recommended_stocks[:10], 1):
            code = s.get('code') or s.get('stock_code') or ''
            name = s.get('name') or s.get('stock_name') or ''
            reason = (s.get('reason') or s.get('推荐理由') or '')[:80]
            lines.append(f"  {i}. {code} {name} — {reason}")
        if len(recommended_stocks) > 10:
            lines.append(f"  ... 还有 {len(recommended_stocks) - 10} 只，详见系统")

        content = '\n'.join(lines)
        subject = f"🐉 智瞰龙虎分析完成 - {date_range}"
        notification_service.send_analysis_result(subject, content)
