...
REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    'morning_portfolio': {
        ...
        'schedule': '10:05 每日',
        'category': '核心', 'default': True, 'core': True,
        'description': '持仓逐只风险分+浮盈+买点+盘中异动；含组合级强力买入/适度买入/持有/适度卖出/优先卖出五档动作；并挑今日 top15 重点候选及早盘 AI 研判',
    },
    'morning_portfolio_ai': {
        'cn': '☀️ 早盘 AI 研判(morning_portfolio 内)',
        'schedule': '10:05 每日(morning_portfolio 内,一次 LLM)',
        'category': '核心', 'default': True,
        'description': '早盘对持仓"风险/买点/异动"子集做一次 LLM 研判(整体基调+重点处理1-2只+早盘纪律);只喂子集控 token、不逐只。关掉则早盘分析回到纯规则',
    },
    'noon_portfolio': {
        'cn': '🕦 午间重点盯盘',
        'schedule': '11:20 每日',
    ...
    'iwencai_openapi_shadow_premarket': {
        'cn': '🔬 问财 OpenAPI 盘前影子采样',
        'schedule': '09:05 交易日',
        'category': '数据', 'default': True,
        'description': '同问句五组独立限额采样；只缓存诊断，不影响正式选股',
    },
    'iwencai_openapi_shadow': {
        'cn': '🔬 问财 OpenAPI 影子对照',
        'schedule': '10:40 起，综合选股完成后',
        'category': '数据', 'default': True,
        'depends_on': ['unified_selection'],
        'dependency_wait_sec': 900,
        'description': '盘前五组采样与旧问财对照；不改变正式候选和通知',
    },
    'iwencai_openapi_shadow_afternoon': {
        'cn': '🔬 问财 OpenAPI 午后影子采样',
        'schedule': '14:45 交易日',
        'category': '数据', 'default': True,
        'depends_on': ['unified_selection'],
        'description': '限额复采五组并附加诊断，正式候选不变',
    },
    'iwencai_openapi_shadow_postclose': {
        'cn': '🔬 问财 OpenAPI 收盘影子采样',
        'schedule': '16:30 交易日',
        'category': '数据', 'default': True,
        'depends_on': ['unified_selection'],
        'description': '限额复采五组并附加诊断，正式候选不变',
