...
import pandas as pd
import numpy as np
import indicators  # 技术指标计算已拆出 data/indicators.py(2026-06-28 阶段3⑤),本类方法委托过去
...
class StockDataFetcher:
    ...
    def _get_chinese_stock_info(self, symbol):
        """获取中国股票基本信息（支持a-stock HTTP > akshare > tushare 优先级）"""
        try:
            # 初始化基本信息
            info = {
                "symbol": symbol,
            ...
            # ★ 优先1: a-stock HTTP直连（最快、最稳定、零依赖）
            try:
                astock_info = self.data_source_manager.get_stock_info_a_stock(symbol)
                if astock_info:
                    info['name'] = astock_info.get('name', '未知')
                    info['current_price'] = astock_info.get('price', 'N/A')
                    info['pe_ratio'] = astock_info.get('pe_ttm', 'N/A')
                    info['pb_ratio'] = astock_info.get('pb', 'N/A')
                    info['industry'] = astock_info.get('industry', '未知')
                    if astock_info.get('mcap_yi'):
                        info['market_cap'] = astock_info['mcap_yi'] * 1e8  # 转为元
                    print(f"[a-stock HTTP] ✅ 成功获取 {symbol} 基本信息")
                    # 如果所有关键字段都有值，直接返回
                    if info['name'] != '未知' and info['pe_ratio'] != 'N/A' and info['current_price'] != 'N/A':
                        # 补充行情数据
                        quote = self.data_source_manager.get_stock_quote_a_stock(symbol)
                        if quote:
                            info['change_percent'] = quote.get('change_pct', 'N/A')
                            info['turnover_pct'] = quote.get('turnover_pct', 'N/A')
                            info['high'] = quote.get('high', 'N/A')
                            info['low'] = quote.get('low', 'N/A')
                            info['open'] = quote.get('open', 'N/A')
                            info['last_close'] = quote.get('last_close', 'N/A')
                        print("[a-stock HTTP] ✅ 数据完整，跳过其他数据源")
                        return info
            except Exception as e:
                print(f"[a-stock HTTP] 获取基本信息失败（将切akshare）: {e}")
            ...
            # 方法1: 尝试获取个股详细信息（akshare，清理代理后调用）
            # 预初始化，防止 pop 抛异常时 finally 触发 UnboundLocalError
            _saved_proxy = None
            _saved_https_proxy = None
            _saved_HTTP_PROXY = None
            _saved_HTTPS_PROXY = None
            try:
                # 清理代理环境变量
                _saved_proxy = os.environ.pop('http_proxy', None)
                ...
                try:
                    ...
                    for ts in ticker_symbols:
                        try:
                            ...
                            short_name = yfinfo.get('shortName') or yfinfo.get('longName')
                            if short_name:
                                info['name'] = short_name
                                print(f"[Yfinance] ✅ 通过yfinance获取股票名称: {short_name}")
                                if yfinfo.get('marketCap'):
                                    info['market_cap'] = yfinfo['marketCap']
                                break
                        except:
                            continue
            ...
            # ★ [a-stock HTTP] 使用腾讯财经接口获取PE/PB（直连，无需代理）
            if info['pe_ratio'] == 'N/A' or info['pb_ratio'] == 'N/A':
                try:
                    quote = self.data_source_manager.get_stock_quote_a_stock(symbol)
                    if quote:
                        if info['pe_ratio'] == 'N/A' and quote.get('pe_ttm'):
                            info['pe_ratio'] = quote['pe_ttm']
                        if info['pb_ratio'] == 'N/A' and quote.get('pb'):
                            info['pb_ratio'] = quote['pb']
                        if info['market_cap'] == 'N/A' and quote.get('mcap_yi'):
                            info['market_cap'] = quote['mcap_yi'] * 1e8
                        if info['current_price'] == 'N/A' and quote.get('price'):
                            info['current_price'] = quote['price']
                        if info.get('turnover_pct') is None and quote.get('turnover_pct'):
                            info['turnover_pct'] = quote['turnover_pct']
                        print(f"[a-stock HTTP] ✅ 成功获取PE/PB: PE={info['pe_ratio']} PB={info['pb_ratio']}")
                except Exception as e:
                    print(f"[a-stock HTTP] 获取PE/PB失败: {e}")
            
            # 方法3: 使用yfinance获取市盈率和市净率（清理代理后尝试akshare百度估值作为备用）
            if info['pe_ratio'] == 'N/A' or info['pb_ratio'] == 'N/A':
                try:
                    import config as _cfg
                    proxy_url = _cfg.PROXY
                    ...
                    yf_tickers = [f"{symbol}.SZ", f"{symbol}.SS"]
                    for ts in yf_tickers:
                        try:
                            ticker = yf.Ticker(ts)
                            yfinfo = ticker.info or {}
                            if info['pe_ratio'] == 'N/A':
                                pe_ttm = yfinfo.get('trailingPE')
                                if pe_ttm and 0 < pe_ttm <= 1000:
                                    info['pe_ratio'] = pe_ttm
                            if info['pb_ratio'] == 'N/A':
                                pb_val = yfinfo.get('priceToBook')
                                if pb_val and 0 < pb_val <= 100:
                                    info['pb_ratio'] = pb_val
                            if info['market_cap'] == 'N/A':
                                mc = yfinfo.get('marketCap')
                                if mc:
                                    info['market_cap'] = mc
                            break
                        except Exception:
                    ...
                    print(f"[Yfinance] ⚠️ 获取估值数据失败: {e}")
            
            # 备用：清理代理后尝试akshare百度估值
            if info['pe_ratio'] == 'N/A':
                _saved_proxy = None
                _saved_https = None
                try:
            ...
            print(f"获取中国股票信息异常: {e}")
            # 不要丢已经从 a-stock HTTP / akshare 拿到的数据
            # 用 info 兜底（若已被填充）；否则才回退到默认空模板
            try:
                if isinstance(info, dict) and info.get('name', '未知') not in ('未知', None, ''):
                    return info
            except NameError:
                pass
            return {
    ...
    def _get_chinese_stock_data(self, symbol, period="1y", adjust="raw"):
        """获取中国 A 股历史日线 —— **委托 datahub.kline**(2026-06-28 阶段3⑤摊平后,K线取数 /
        三级缓存 / 多源健康度降级全部收口 datahub,打破旧 datahub→fetcher→manager 嵌套与重复路由)。

        adjust='raw'(默认):不复权(回测/持仓/真实价);'qfq':前复权(技术分析,datahub 统一 qfq 源)。
        返回 DatetimeIndex='Date' + 大写 OCHLV 的 DataFrame;失败返回 {'error': ...}(沿用旧契约,
        177 调用方多以 isinstance(df, dict) 判失败)。与 datahub.kline 共用 db/kline_cache 磁盘缓存。
        ⚠️ datahub.kline 的 raw 源链已不含本 fetcher,无循环回调。"""
        adjust = 'qfq' if str(adjust) == 'qfq' else 'raw'
        try:
            import datahub
            df = datahub.kline(symbol, period, '1d', adjust=adjust)
    ...
    def calculate_technical_indicators(self, df):
        """计算技术指标(委托 data/indicators.py;2026-06-28 阶段3⑤拆出,逐字段口径不变)。"""
        return indicators.calculate_technical_indicators(df)

    def get_latest_indicators(self, df):
        """获取最新的技术指标值(委托 data/indicators.py;2026-06-28 阶段3⑤拆出,逐字段口径不变)。"""
        return indicators.get_latest_indicators(df)
