...
def _setup_stdout_encoding():
    """在 Windows 命令行尽量使用 UTF-8，不依赖已退役的 UI 运行时。"""
...
class MarketSentimentDataFetcher:
    ...
    def _interpret_market_sentiment(self, change_pct):
        """根据涨跌幅解读市场情绪"""
        ...
        try:
            ...
            if pct > 2:
                return "市场情绪亢奋，指数大幅上涨"
            elif pct > 0.5:
                return "市场情绪积极，指数小幅上涨"
            elif pct > 0:
                return "市场情绪平稳，指数微涨"
            elif pct > -0.5:
                return "市场情绪谨慎，指数微跌"
    ...
    def _get_turnover_rate(self, symbol):
        """获取换手率数据（a-stock HTTP > akshare > tushare 优先级）"""
        try:
            # ★ 优先级1: a-stock HTTP直连（腾讯财经，零依赖）
            try:
                print(f"   [a-stock HTTP] 正在获取换手率数据...")
                quote = data_source_manager.get_stock_quote_a_stock(symbol)
                if quote and quote.get('turnover_pct'):
                    turnover_rate = quote['turnover_pct']
                    ...
                    print(f"   [a-stock HTTP] ✅ 成功获取换手率: {turnover_rate}%")
                    return {
    ...
    def _get_market_index_sentiment(self):
        """获取大盘指数情绪（a-stock HTTP > akshare > tushare 优先级）"""
        try:
            # ★ 优先级1: a-stock HTTP直连（腾讯财经，零依赖）
            try:
                print(f"   [a-stock HTTP] 正在获取大盘指数数据...")
                from a_stock_data_adapter import _tencent_quote
                # 查询主要指数
                idx_quotes = _tencent_quote(["000001", "000300", "399006", "000688"])
                if idx_quotes:
                    # 构建返回数据
                    sh = idx_quotes.get("000001", {})
                    sz = idx_quotes.get("399001", {})
                    cy = idx_quotes.get("399006", {})
                    kcb = idx_quotes.get("000688", {})
                    
                    return {
                        "data": {
                            "上证指数": {
                                "current": sh.get('price', 0),
                                "change_pct": sh.get('change_pct', 0),
                                "name": sh.get('name', '上证指数'),
                            },
                            "深证成指": {
                                "current": sz.get('price', 0) if sz else 0,
                                "change_pct": sz.get('change_pct', 0) if sz else 0,
                                "name": sz.get('name', '深证成指') if sz else '深证成指',
                            },
                            "创业板指": {
                                "current": cy.get('price', 0),
                                "change_pct": cy.get('change_pct', 0),
                                "name": cy.get('name', '创业板指'),
                            },
                            "科创50": {
                                "current": kcb.get('price', 0),
                                "change_pct": kcb.get('change_pct', 0),
                                "name": kcb.get('name', '科创50'),
                            },
                            "沪深300": {
                                "current": sh.get('price', 0),
                                "change_pct": sh.get('change_pct', 0),
                                "name": sh.get('name', '沪深300'),
                            },
                        },
                        "interpretation": self._interpret_market_sentiment(sh.get('change_pct', 0)),
                        "source": "a-stock HTTP"
                    }
            except Exception as ae:
                print(f"   [a-stock HTTP] ❌ 获取失败（将切akshare）: {ae}")
            
            # 优先级2: 使用akshare获取上证指数实时数据
    ...
    def _get_margin_trading_data(self, symbol):
        """获取融资融券数据（a-stock HTTP > akshare 优先级）"""
        try:
            # ★ 优先级1: a-stock HTTP直连（东财datacenter，零依赖）
            try:
                print(f"   [a-stock HTTP] 正在获取融资融券数据...")
                margin_data = data_source_manager.get_margin_trading_a_stock(symbol, 5)
                if margin_data and len(margin_data) > 0:
                    latest = margin_data[0]
                    margin_balance = latest.get('rzye', 0) / 1e8  # 转为亿元
                    short_balance = latest.get('rqye', 0) / 1e8
                    ...
                    print(f"   [a-stock HTTP] ✅ 成功获取融资融券数据")
                    return {
                        "margin_balance": f"{margin_balance:.2f}",
                        "short_balance": f"{short_balance:.2f}",
                ...
                print(f"   [a-stock HTTP] ❌ 获取融资融券失败（将切akshare）: {ae}")
            
            # 优先级2: 获取个股融资融券数据（尝试多个API）
            try:
                # 方法1：获取沪深融资融券明细
                _throttle('akshare')
