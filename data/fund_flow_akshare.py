...
class FundFlowAkshareDataFetcher:
    ...
    def _get_individual_fund_flow(self, symbol, market):
        """获取个股资金流向数据（a-stock HTTP > akshare > tushare 优先级）"""
        try:
            # ★ 优先级1: a-stock HTTP直连（东财push2his，零依赖）
            try:
                print(f"   [a-stock HTTP] 正在获取资金流向 (东财push2his)...")
                flow_data = data_source_manager.get_fund_flow_a_stock(symbol, self.days)
                if flow_data and len(flow_data) > 0:
                    print(f"   [a-stock HTTP] ✅ 成功获取 {len(flow_data)} 条资金流向数据")
                    # 转换为与akshare兼容的格式
                    data_list = []
                    for row in flow_data:
                        item = {
                            '日期': row.get('date', ''),
                            '主力净流入-净额': row.get('main_net', 0),
                            '小单净流入-净额': row.get('small_net', 0),
                            '中单净流入-净额': row.get('mid_net', 0),
                            '大单净流入-净额': row.get('large_net', 0),
                            '超大单净流入-净额': row.get('super_net', 0),
                        }
                        data_list.append(item)
                    return {
                        "data": data_list,
                        "days": len(data_list),
                        "columns": ['日期', '主力净流入-净额', '小单净流入-净额', '中单净流入-净额', '大单净流入-净额', '超大单净流入-净额'],
                        "market": market,
                        "source": "a-stock HTTP",
            ...
            # 优先级2: 使用akshare的stock_individual_fund_flow接口
