...
class StockAnalysisAgents:
    ...
    def technical_analyst_agent(self, stock_info: Dict, stock_data: Any, indicators: Dict,
                                chan_data: Dict = None) -> Dict[str, Any]:
        """技术面分析智能体

        chan_data: 可选的缠论分析结果（chan_theory.analyze_chan 输出）。
                   未提供时自动基于 stock_data(K线 DataFrame) 计算，失败则静默降级。
        """
        print("🔍 技术分析师正在分析中...")

        # 缠论结构（借鉴 SkillHub 缠论 skill，纯本地计算，失败不影响主流程）
        chan_summary = None
        try:
            if chan_data is None:
                from chan_theory import analyze_chan
                chan_data = analyze_chan(stock_data, stock_info.get('symbol', ''))
            if isinstance(chan_data, dict):
                chan_summary = chan_data.get('summary')
            if chan_summary:
                print(f"   ✓ 缠论结构已计算（{chan_data.get('bi_count', 0)}笔/"
                      f"{chan_data.get('zhongshu_count', 0)}中枢，买卖点："
                      f"{(chan_data.get('buy_sell_point') or {}).get('signal', '无')}）")
    ...
    def risk_management_agent(self, stock_info: Dict, indicators: Dict, risk_data: Dict = None,
                              stock_data: Any = None) -> Dict[str, Any]:
        """风险管理智能体（增强版）

        stock_data: 可选 K线 DataFrame，用于计算量化风险(VaR/CVaR/最大回撤/蒙特卡洛/压力测试)。
                    失败则静默降级，不影响原有风险评估。
        """
        print("⚠️ 风险管理师正在评估中...")
        ...
        # 量化风险（借鉴 SkillHub 风险压力测试 skill，纯本地计算）
        quant_risk = None
        quant_risk_text = ""
        try:
            if stock_data is not None:
                ...
                quant_risk = analyze_risk(stock_data, beta=beta)
                if quant_risk.get('available'):
                    quant_risk_text = f"\n【量化风险指标】（VaR/CVaR/回撤/蒙特卡洛/压力测试，本地计算）\n{quant_risk['summary']}\n"
                    print(f"   ✓ 量化风险已计算（VaR95 {quant_risk['var_hist']*100:.2f}% / 最大回撤 "
                          f"{quant_risk['max_drawdown'].get('max_drawdown', float('nan'))*100:.1f}%）")
        except Exception as e:
            print(f"   ⚠ 量化风险计算跳过：{e}")
...
"""
        ...
        # ⭐ 缓存优化(2026-07-17):稳定框架(角色+处理步骤+11维风险分析口径+输出要求)放 system——
        # 全批同 call_type 逐字相同 → DeepSeek 前缀缓存命中;逐股变量(股票信息/技术指标/风险数据)放 user。
        # 原来整套 ~75 行框架塞在 user、且逐股数据拼在框架之前 → 前缀从"股票代码"就发散、框架永不命中
        # (与 technical_analysis 5%→86% 同型缺陷)。
        system = """你是一名资深的风险管理专家，具有20年以上的风险识别和控制经验，擅长全面评估各类投资风险，特别关注限售解禁、股东减持、重要事件等可能影响股价的风险因素。你擅长从海量原始数据中提取关键信息，进行深度解析和量化评估。

请基于 user 提供的【股票信息/技术指标/风险数据】进行全面深度的风险评估。

⚠️ 重要提示：user 提供的风险数据是从问财（pywencai）实时查询的完整原始数据，请你：
1. 仔细解析每一条记录的所有字段信息
...
1. **限售解禁风险分析** ⭐ 重点
   - 解禁时间和规模评估 / 解禁对股价的潜在冲击 / 解禁股东类型分析（创始人/投资机构/其他）
   - 历史解禁后股价走势参考 / 风险等级评定和应对建议
2. **股东减持风险分析** ⭐ 重点
   - 减持频率和力度评估 / 减持股东身份和意图分析 / 减持对市场信心的影响
   - 是否存在连续减持或集中减持 / 风险警示和投资建议
3. **重要事件风险分析** ⭐ 重点
   - 识别可能影响股价的重大事件 / 事件性质判断（利好/利空/中性）
   - 事件影响的时间维度（短期/中期/长期）/ 事件的确定性和不确定性 / 风险提示和关注要点
4. **市场风险（系统性风险）**：宏观经济环境 / 市场整体走势 / Beta 系数反映的市场敏感度 / 系统性风险应对策略
5. **个股风险（非系统性风险）**：公司基本面 / 经营管理 / 竞争力 / 行业地位风险
6. **流动性风险**：成交量和换手率 / 买卖盘深度 / 流动性枯竭风险 / 大额交易影响
7. **波动性风险**：价格波动幅度 / 52周最高最低位 / RSI 等技术指标的风险提示 / 波动率对投资的影响
8. **估值风险**：当前估值水平 / 市场预期和估值偏差 / 估值过高风险警示
9. **行业风险**：行业周期阶段 / 竞争格局 / 政策风险 / 技术变革风险
10. **综合风险评定**：风险等级（低/中/高）/ 主要风险因素排序 / 风险暴露时间窗口 / 风险演变趋势判断
11. **风险控制建议** ⭐ 核心：仓位控制（具体比例）/ 止损位（具体价位）/ 风险规避策略（什么情况下不建议投资）/ 风险对冲方案 / 持仓时间建议 / 重点关注指标和信号

请基于实际数据进行客观、专业、严谨的风险评估，给出可操作的风险控制建议。如果某些风险数据缺失，也要指出数据缺失本身可能带来的风险。"""
...
- 布林带位置：当前价格相对于上下轨的位置
- 波动率指标等
{quant_risk_text}{risk_data_text}"""
        ...
        analysis = self.deepseek_client.call_api(messages, max_tokens=6000, call_type='risk')
        ...
        return {
            ...
            "quant_risk": quant_risk,  # VaR/CVaR/回撤/蒙特卡洛/压力测试
            "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")
        }
...
"""
        ...
        analysis = self.deepseek_client.call_api(messages, max_tokens=4000, call_type='sentiment')
...
"""
        ...
        analysis = self.deepseek_client.call_api(messages, max_tokens=4000, call_type='news')
    ...
    def run_multi_agent_analysis(self, stock_info: Dict, stock_data: Any, indicators: Dict, 
        ...
        # 风险管理分析（传入风险数据 + K线用于量化风险）
        if enabled_analysts.get('risk', True):
            agents_results["risk_management"] = self.risk_management_agent(stock_info, indicators, risk_data, stock_data=stock_data)
...
"""
        ...
        discussion_result = self.deepseek_client.call_api(messages, max_tokens=6000, call_type='discussion')
        
        print("✅ 团队讨论完成")
        return discussion_result
    
    def investment_philosophy_agent(self, stock_info: Dict, discussion_result: str,
                                    lenses=None) -> Dict[str, Any]:
        """投资哲学透镜(借鉴妙想 巴菲特/段永平/纳瓦尔/韭菜 思维 skill)。

        在多智能体共识之上做一次价值投资 + 长期视角 + 韭菜反向 的高层复核。
        失败静默降级(不影响最终决策)。
        """
        print("🧭 投资哲学透镜复核中(价值视角 + 韭菜反向校验)...")
        try:
            from investment_lens import build_review_prompt
            brief = (f"代码:{stock_info.get('symbol','N/A')} 名称:{stock_info.get('name','N/A')} "
                     f"现价:{stock_info.get('current_price','N/A')} PE:{stock_info.get('pe_ratio','N/A')} "
                     f"PB:{stock_info.get('pb_ratio','N/A')} 行业:{stock_info.get('sector','N/A')}")
            prompt = build_review_prompt(brief, (discussion_result or '')[:3000], lenses)
            messages = [
                {"role": "system", "content": "你是资深投资哲学顾问,擅长用投资大师的思维框架做高层决策复核与人性偏差识别。"},
                {"role": "user", "content": prompt},
            ]
            review = self.deepseek_client.call_api(messages, max_tokens=2000, call_type='philosophy')
            return {"agent_name": "投资哲学透镜", "review": review,
                    "timestamp": time.strftime("%Y-%m-%d %H:%M:%S")}
        except Exception as e:
            print(f"   ⚠ 投资哲学透镜跳过：{e}")
            return {"agent_name": "投资哲学透镜", "review": None, "error": str(e)}

    def make_final_decision(self, discussion_result: str, stock_info: Dict, indicators: Dict,
                            with_philosophy: bool = True) -> Dict[str, Any]:
        """制定最终投资决策

        with_philosophy: 决策前先跑投资哲学透镜,并把其复核意见注入最终决策(默认开)。
        """
        print("📋 正在制定最终投资决策...")

        philosophy_review = None
        if with_philosophy:
            ph = self.investment_philosophy_agent(stock_info, discussion_result)
            philosophy_review = ph.get('review')

        time.sleep(1)
        decision = self.deepseek_client.final_decision(
            discussion_result, stock_info, indicators, philosophy=philosophy_review)

        if isinstance(decision, dict) and philosophy_review:
            decision['philosophy_review'] = philosophy_review
        print("✅ 最终投资决策完成")
        return decision
