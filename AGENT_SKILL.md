...
- `decision_signal_transitions(code,source_type,days,material_only)` — 服务端动作变化；默认仅实质风险状态切换
...
- **盘中决策快照**:`intraday_decision_snapshot()` 只读今日已落库产物，不现拉数据；分别返回全部真实持仓、正式 TOP5、TOP15 观察池、问财参考及批量行情覆盖/as-of。价格或时点无效时必须尊重 `data_quality` 与“暂不给价”。
...
客户端配 `{ "command":"python", "args":["<项目根>/mcp_server.py"] }`，按需从仓库外传入模型和外部服务配置。
工具按数据、计算、选股、组合、基金、重分析和控制面分组；具体数量以 MCP 运行时能力清单为准，不在文档中维护易过期的计数。
- Agent 控制面:
  ...
  - `latest_premarket_facts(report_date)`：只读盘前 CLS 红讯事实、双窗口标签、稳定题材线程、证据 ID 和分阶段降级；不现抓、不调用 LLM，永久 `reference_only`。
...
- ⏳ 未做:成交记录列名中文化;`event_scoring`/`report_templates` 接入业务。
