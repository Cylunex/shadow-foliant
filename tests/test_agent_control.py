...
class ScheduledDependencyTests(unittest.TestCase):
    ...
    def test_formal_selection_failure_does_not_run_external_candidate_fallback(self):
        ...
        expected_selection_day = datetime.now().date().isoformat()
        refresh.assert_called_once_with(expected_selection_day)
        fallback.assert_not_called()
        log_run.assert_not_called()

    def test_monday_preopen_refreshes_calendar_through_selection_day(self):
        ...
        with mock.patch('analysis.local_stock_selector.LocalStockSelector',
                           return_value=selector), \
                mock.patch('core.decision_context.DecisionContext.build',
                           return_value=context):
            returned_selector, returned_context, trade_date = (
                jobs_hub._preopen_research_context(syncer, '2026-08-31')
            )

        self.assertIs(returned_selector, selector)
        self.assertIs(returned_context, context)
        self.assertEqual(trade_date, '2026-08-28')
        syncer.refresh_calendar_for_day.assert_called_once_with('2026-08-31')
