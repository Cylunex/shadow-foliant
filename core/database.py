...
class StockAnalysisDatabase:
    def __init__(self, db_path=_bootstrap.db_path("stock_analysis.db")):
        """初始化数据库连接"""
...
# 全局数据库实例 — 通过 .env 的 USE_POSTGRES=true 切换到 PostgreSQL 后端
if os.getenv('USE_POSTGRES', '').lower() in ('1', 'true', 'yes', 'on'):
    try:
        from database_pg import db as _pg_db
        db = _pg_db
        print('[Database] 已切换到 PostgreSQL 后端 (analysis_records)')
    except Exception as _e:
        print(f'[Database] PG 切换失败，回退到 SQLite: {_e}')
