...
SCHEMA: List[Dict[str, str]] = [
    ...
    {'key': 'ZZSHARE_TOKEN', 'label': 'zzshare Token', 'group': '数据源', 'type': 'secret',
     'help': 'A股日线/分钟线正式 API；只保存在仓库外 .env'},
    {'key': 'ZZSHARE_ENABLED', 'label': '启用 zzshare', 'group': '数据源', 'type': 'bool',
     'help': '有 Token 时作为分钟线首选源'},
    {'key': 'FUYAO_AICUBES_API_KEY', 'label': '扶摇同花顺 API Key', 'group': '数据源', 'type': 'secret',
     'help': '推荐由仓库外受限文件注入；不得写入仓库或日志'},
    {'key': 'FUYAO_AICUBES_API_KEY_FILE', 'label': '扶摇密钥文件', 'group': '数据源', 'type': 'text',
     'help': 'dotenv 文件绝对路径；读取键名默认为 fuyao-aicubes'},
    {'key': 'FUYAO_AICUBES_ENABLED', 'label': '启用扶摇同花顺', 'group': '数据源', 'type': 'bool',
     'help': '正式行情、日线、日历与基本面源；无密钥自动降级'},
    {'key': 'FUYAO_AICUBES_PRIORITY_TIER', 'label': '扶摇路由优先级', 'group': '数据源', 'type': 'int',
     'help': '0-3，默认 0；同级仍按运行健康度排序'},
    {'key': 'TDX_USE_ELTDX', 'label': '启用 eltdx', 'group': '数据源', 'type': 'bool',
    ...
    {'key': 'TDX_USE_TDX_PYTHON', 'label': '启用 tdx-python', 'group': '数据源', 'type': 'bool',
     'help': '仅 Python 3.12+ 可用的增强 TDX 兜底'},
    {'key': 'TDX_PYTHON_ADDRESS', 'label': '固定 TDX 地址', 'group': '数据源', 'type': 'text',
     'help': '可选；留空由 SDK 自动选择，只能保存在受限配置'},
    {'key': 'EASY_TDX_HOST', 'label': '固定 TDX 节点', 'group': '数据源', 'type': 'text',
     'help': '可选；留空使用 easy-tdx 自动选优，不应提交到仓库'},
    {'key': 'MARKET_DATA_MAX_BARS', 'label': '单次最大 K 线数', 'group': '数据源', 'type': 'int',
     'help': '默认 3200，TDX 自动按 800 条分页'},
    {'key': 'TDX_USE_MOOTDX', 'label': '启用 mootdx(通达信公网)', 'group': '数据源', 'type': 'bool',
     'help': '旧兼容兜底；新部署优先 eltdx'},
