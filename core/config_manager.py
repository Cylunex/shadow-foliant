...
class ConfigManager:
    ...
    def __init__(self, env_file: str = ".env"):
        ...
        self.default_config = {
            ...
            "DEEPSEEK_BASE_URL": {
                "value": "",
                "description": "LLM API地址（如方舟Ark: https://ark.cn-beijing.volces.com/api/coding/v3）",
                "required": True,
                "type": "text"
            },
            "DEFAULT_MODEL_NAME": {
                "value": "",
                "description": "AI模型名称（如 deepseek-v4-pro）",
                "required": True,
                "type": "text"
            },
            "TUSHARE_TOKEN": {
    ...
    def write_env(self, config: Dict[str, str]) -> bool:
        ...
        try:
            ...
            lines.append(f'DEEPSEEK_BASE_URL="{config.get("DEEPSEEK_BASE_URL", "")}"')
            lines.append(f'DEFAULT_MODEL_NAME="{config.get("DEFAULT_MODEL_NAME", "")}"')
            lines.append("")
            ...
            lines.append(f'PG_USER="{config.get("PG_USER", "aiagents_stock")}"')
            lines.append(f'PG_PASSWORD="{config.get("PG_PASSWORD", "changeme")}"')
            lines.append("")
